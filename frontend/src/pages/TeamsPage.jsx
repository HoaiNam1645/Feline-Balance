...
/* ── Team Form Modal ── */
function TeamFormModal({ isOpen, onClose, onSubmit, formData, onChange, title, submitLabel }) {
    if (!isOpen) return null;
    return (
        <div style={{
            position: 'fixed', inset: 0, zIndex: 9999,
...
export default function TeamsPage() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const url = isEdit ? `${API_BASE}/api/teams/${editingId}` : `${API_BASE}/api/teams`;
    ...
    const handleDelete = async () => {
        try {
            const res = await fetch(`${API_BASE}/api/teams/${deletingId}`, { method: 'DELETE' });
            const json = await res.json();
            if (json.success) {
    ...
    return (
        <>
            <Topbar onRefresh={fetchTeams} loading={loading} />
            <div className="page-content">

                {/* Header */}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                    <div>
                        <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '4px' }}>Management / Teams</div>
                        <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 700, color: 'var(--text-primary)' }}>Team Management</h2>
                    </div>
                ...
                <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(2, 1fr)', marginBottom: '24px' }}>
                    <div className="stat-card">
                        <div className="stat-card-header">
                            ...
                            <span className="stat-card-label">Total Teams</span>
                        </div>
                        <div className="stat-card-value">{teams.length}</div>
                        <div className="stat-card-sub">Active teams in system</div>
                    </div>
                    <div className="stat-card">
                        <div className="stat-card-header">
                ...
                <div className="table-container">
                    {loading ? (
                        ...
                        <div className="table-empty">
                            ...
                            <h3>No teams yet</h3>
                            <p>Click "New Team" to create your first team.</p>
                        </div>
                    ) : (
                                ...
                                <tbody>
                                    {teams.map((team, idx) => (
                                        <tr key={team.id}>
                                            ...
                                            <td>
                                                <span style={{ color: 'var(--text-secondary)', fontSize: '13px' }}>{team.description || '—'}</span>
                                            </td>
                                            <td>
                                                <span style={{ fontSize: '13px', color: 'var(--text-muted)' }}>
                                                    {team.created_at ? new Date(team.created_at).toLocaleString('vi-VN', {
            ...
            <TeamFormModal
                ...
                title={modalMode === 'create' ? 'New Team' : 'Update Team'}
